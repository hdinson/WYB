...
import kotlinx.android.synthetic.main.item_living_window_shop.view.*
 ...
 * 直播间橱窗商品
 */
class LivingWindowShopAdapter(dataList: List<WindowShopGoods>) : CommonAdapter<WindowShopGoods>(dataList) {

    override fun getLayoutId(viewType: Int) = R.layout.item_living_window_shop
    ...
    override fun convert(holder: CommonViewHolder, dataBean: WindowShopGoods, position: Int) {
        ...
        holder.itemView.addMinusView.setCurrentNum(dataBean.selectGoodsNum)
        holder.itemView.addMinusView.setOnNumberChangeListener(OnNumberChangeListener {
            dataBean.selectGoodsNum = it
            EventBus.getDefault()
        ...
        GlideUtils.loadImage(holder.itemView.context,dataBean.imageUrl,into = holder.itemView.ivGoods)
    }
